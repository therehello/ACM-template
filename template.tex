...
struct Persistent_seg {
    ...
    int update(int rt, const int& idx, const int& val, int l, int r) {
        ...
        if (idx <= mid)
            ls = update(ls, idx, val, l, mid);
        else
            rs = update(rs, idx, val, mid + 1, r);
        tree.push_back({ls, rs, tree[ls].val + tree[rs].val});
        return tree.size() - 1;
    }
    int query(int rt1, int rt2, int k, int l, int r) {
        ...
        if (k <= lcnt)
            return query(tree[rt1].ls, tree[rt2].ls, k, l, mid);
        else
            return query(tree[rt1].rs, tree[rt2].rs, k - lcnt, mid + 1, r);
    }
};
\end{lstlisting}
...
struct frac {
    ...
    frac operator-(const frac& other) { return -other + *this; }
    frac operator*(const frac& other) { return frac(a * other.a, b * other.b); }
    frac operator/(const frac& other) {
        assert(other.a);
        return *this * frac(other.b, other.a);
    }
    bool operator<(const frac& other) { return (*this - other).a < 0; }
    bool operator<=(const frac& other) { return (*this - other).a <= 0; }
    bool operator>(const frac& other) { return (*this - other).a > 0; }
    bool operator>=(const frac& other) { return (*this - other).a >= 0; }
    bool operator==(const frac& other) { return a == other.a && b == other.b; }
